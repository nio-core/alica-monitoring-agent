...
TEST(AllocationAuthorityInfoFromCapnp, entrypoint_robots_is_set_properly) {
    ...
    auto allocationAuthorityInfo = AllocationAuthorityInfo::from(reader);
    EXPECT_EQ(allocationAuthorityInfo.getEntrypointRobots().size(), ENTRY_POINT_COUNT);
    for(const auto& entrypoint: allocationAuthorityInfo.getEntrypointRobots()) {
        EXPECT_EQ(entrypoint.getEntrypoint(), ENTRY_POINT);
        EXPECT_EQ(entrypoint.getRobots().size(), ROBOT_COUNT);
        for(auto robot: entrypoint.getRobots()) {
