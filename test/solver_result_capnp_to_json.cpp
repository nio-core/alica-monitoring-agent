...
TEST(SolverResultCapnprotoToJson, with_missing_fields_can_not_be_parsed) {
    ...
    EXPECT_THROW(processing::try_read_solver_result(reader), std::runtime_error);
}
...
TEST(SolverResultCapnprotoToJson, it_can_be_parsed) {
    ...
    EXPECT_NO_THROW(processing::try_read_solver_result(reader));
}
...
TEST(SolverResultCapnprotoToJson, it_contains_sender_id) {
    ...
    const std::string json = processing::try_read_solver_result(reader);
